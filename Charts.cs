...
namespace UrhoSharpSample
{
		...
		async void CreateScene()
		{
			...
			for (var i = 0f; i < size * 1.5f; i += 1.5f)
			{
				for (var j = 0f; j < size * 1.5f; j += 1.5f)
				{
					...
					var box = new Bar(new Color(Color.Gray));
	...
	public class Bar : Component
	{
		...
		public float HeightValue
		{
			...
			set { barNode.Scale = new Vector3(WidthValue, value < 0.3f ? 0.3f : value, DepthValue); }
		}

		public float WidthValue
		{
			get { return barNode.Scale.X; }
			set { barNode.Scale = new Vector3(value < 0.3f ? 0.3f : value, HeightValue, DepthValue); }
		}

		public float DepthValue
		{
			get { return barNode.Scale.Z; }
			set { barNode.Scale = new Vector3(WidthValue, HeightValue, value < 0.3f ? 0.3f : value); }
		}
